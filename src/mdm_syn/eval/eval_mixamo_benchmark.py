...
from src.mdm_syn.train.train_platforms import ClearmlPlatform, TensorboardPlatform, NoPlatform  
...
if __name__ == '__main__':
    ...
    for sample_i in range(args.benchmark_size):
        for iter in eval_dicts[f'{sample_i:04d}']:
            for m, vals in eval_dicts[f'{sample_i:04d}'][iter].items():
                train_platform.report_scalar(name=f'sample{sample_i:04d}', value=vals['mean'], 
                                             iteration=iter, group_name=m)
    ...
    for iter in eval_dicts['0000'].keys():
        mean_dict[iter] = {
            m: {
                'mean': np.mean([eval_dicts[f'{sample_i:04d}'][iter][m]['mean'] for sample_i in range(args.benchmark_size)]),
                'std': np.std([eval_dicts[f'{sample_i:04d}'][iter][m]['mean'] for sample_i in range(args.benchmark_size)]),
