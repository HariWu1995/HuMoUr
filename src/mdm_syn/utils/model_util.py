from src.mdm_syn.diffusion import gaussian_diffusion as gd
from src.mdm_syn.diffusion.respace import SpacedDiffusion, space_timesteps

from src.mdm_syn.models.mdm_qnanet import QnAMDM
from src.mdm_syn.models.mdm_unet import MDM_UNetModel
...
def create_model_and_diffusion(args, data, num_joints = None):
    ...
    if args.arch == 'unet':
        ...
        raise ValueError(
            "Architecture specified not supported: %s" % (args.arch,))
...
def get_model_args(args, data, num_joints):
    ...
    if data is not None and hasattr(data, 'dataset') \
                        and hasattr(data.dataset, 'num_actions'):
