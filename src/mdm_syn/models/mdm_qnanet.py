...
from src.mdm_syn.models.qna import FusedQnA1d, FusedUpQnA1d
...
class QnABlock2d(nn.Module):
    def __init__(self,
        ...
        self.drop_path1 = (StochasticDepth(self.drop_path_prob, mode="row") if self.drop_path_prob > 0.0 else nn.Identity())
        ...
        self.drop_path2 = (StochasticDepth(self.drop_path_prob, mode="row") if self.drop_path_prob > 0.0 else nn.Identity())
        self.skip_gate = nn.Sequential(nn.SiLU(),
...
class QnAMDM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert (num_layers % 2 == 0 or num_downsample == 0), \
        ...
        for i in range(num_downsample + 1):
            ...
            self.hierarchy_downsample[f"h{i}"] = (
                Downsample(self.latent_dim * (2 ** i), 
                           self.latent_dim * (2 ** (i + 1))) if i != num_downsample else
                CustomIdentity()
            )
