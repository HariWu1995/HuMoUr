...
class FusedQnA(nn.Module):
    ...
    def _compute_attention(self, cost, v, rpe=None):
        """
        Compute the attention in memory efficent manner 
        (see paper: https://arxiv.org/abs/2112.11435)
        """
        B, _, H, W = v.shape
        ...
        I = v_cost_exp
        sum_v_cost_exp = F.conv2d(I, summation_kernel, stride=self.stride, padding=self.padding, groups=self.hidden_features)
        ...
        summation_kernel = rpe_exp if rpe is not None else \
                            torch.ones(self.heads, 1, self.kernel_size, self.kernel_size).to(v_cost_exp)
...
class FusedQnA1d(nn.Module):
    ...
    def _compute_attention(self, cost, v, rpe=None):
        ...
        I = v_cost_exp
        sum_v_cost_exp = F.conv2d(I, summation_kernel, stride=(1, self.stride), padding=(0, self.padding), groups=self.hidden_features)
...
class FusedUpQnA1d(nn.Module):
    ...
    def _compute_attention(self, cost, v, rpe=None):
        ...
        v_cost_exp = cost_exp[:, :, :, None, ...] * v.view(B, 1, self.heads, h_dim, H, W)  # [B, q, heads, head_dim, H, W]
        ...
        if rpe is not None:
            ...
            summation_kernel = torch.ones(self.scale_factor * self.hidden_features, 1, self.axial_dim, self.kernel_size).to(v_cost_exp)
        ...
        summation_kernel = (
            ...
            if rpe is not None 
            else torch.ones(self.heads, 1, self.axial_dim, self.kernel_size).to(v_cost_exp)
        )
        sum_cost_exp = F.conv2d(I,
                                summation_kernel,
                                stride=(1, self.stride),
    ...
    def forward(
        ...
        if self.timesteps_embed is not None and timesteps is not None:
            q = self.timesteps_embed(timesteps)
            q = q.view(B, self.scale_factor, self.heads, self.hidden_features // self.heads)
        else:
            # Prepare query
            # assert False, "Don't unset timesteps unless explicitly done for testing"
            q = torch.broadcast_to(self.query, [x.shape[0], self.scale_factor * self.hidden_features])
            q = q.view(B, self.scale_factor, self.heads, self.hidden_features // self.heads)
