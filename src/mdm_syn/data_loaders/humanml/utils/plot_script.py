...
def plot_3d_motion(save_path, kinematic_tree, joints, title, dataset, figsize=(3, 3), fps=120, radius=3,
    ...
    frame_colors = ['blue' if index in gt_frames else \
                    'orange' for index in range(frames_number)]
...
def explicit_plot_3d_motion(save_path, kinematic_tree, joints, title, dataset, figsize=(3, 3), fps=120, radius=3, vis_mode="default", frame_colors=[]):
    ...
    def update(index):
        ...
        plot_xzPlane(MINS[0] - trajec[index, 0], 
                     MAXS[0] - trajec[index, 0], 0, 
                     MINS[2] - trajec[index, 1], 
                     MAXS[2] - trajec[index, 1], )

        used_colors = colors_dict[frame_colors[index]] if (index < len(frame_colors)) else \
                      colors_dict["blue"]
