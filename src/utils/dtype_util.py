...
def convert_dataset_fbx_to_bvh(in_dir, out_dir = None):
    """
    This code is a variation of 
        https://github.com/rubenvillegas/cvpr2018nkn/blob/master/datasets/fbx2bvh.py
    """
    # in_dir = '<path to folder containing fbx file>'
    # out_dir = in_dir + '/fbx2bvh'  
    # # out_dir = in_dir.replace('fbx', 'bvh')

    if out_dir is None:
        out_dir = os.path.join(in_dir, 'fbx2bvh')
    ...
    for idx, in_file in pbar:
        ...
        fbx_to_bvh(in_file, out_file)


def convert_dataset_mdm_to_bvh(input_bvh, motion_path):
