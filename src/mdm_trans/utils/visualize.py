...
def plot_3d_motion_features(kinematic_tree, joints, title, dataset, figsize=(5, 3), fps=120, radius=3,
    ...
    def update(index):
        ...
        if feat_joints_pca is not None:
            joint_colors = feat_per_joint_colors[index] if feat_per_joint_colors.ndim == 3 else \
                           feat_per_joint_colors
            ax.scatter(xs=data[index, :, 0], 
                       ys=data[index, :, 1], 
                       zs=data[index, :, 2], c=joint_colors, marker='o', s=81)
...
def plot_3d_motion_attention(attention_dict, kinematic_tree, joints, title, dataset, 
                             figsize=(5, 3), fps=120, radius=3, do_stretch=True):
    matplotlib.use('Agg')
...
def save_multiple_samples(out_path, file_templates,  animations, 
                          fps, max_frames, n_rows_in_out_file=3, *args, **kwargs):
