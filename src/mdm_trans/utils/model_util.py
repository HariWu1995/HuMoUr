...
from src.mdm_trans.model.mdm import MDM
from src.mdm_trans.utils.misc import recursive_op1, tensor_to_device

from src.mdm_trans.data_loaders.tensors import collate

from src.mdm_trans.diffusion import gaussian_diffusion as gd
from src.mdm_trans.diffusion.respace import SpacedDiffusion, space_timesteps
...
def load_model_wo_clip(model, state_dict):
    ...
    missing_keys, \
    unexpected_keys = model.load_state_dict(state_dict, strict=False)
    
    # backward compitability 
    # TODO: remove after a new model is trained
    # self_frame_attn is equal to self_attn so not harm done
    unexpected_keys = [key for key in unexpected_keys 
                           if 'self_frame_attn' not in key] 
    
    assert len(unexpected_keys) == 0
    assert all([k.startswith('clip_model.') or \
                k.startswith('bert_model.') or \
                    'sequence_pos_encoder' in k 
...
def get_model_args(args, data, additional_args={}):
    ...
    model_args =  {
        ...
        'root_sep_io': args.root_sep_io, 
        'frame_averaging': args.frame_averaging,
        'diffusion_steps': args.diffusion_steps, 
        'agg_features': getattr(args, 'agg_features', False),
    }
...
def load_into_model_format(motion_paths, normalizer_class, load_motions_func):
    ...
    collate_args = [{'inp': motion, 
                     'tokens': None, 
                     'lengths': len, 
                     'text': txt,} for motion, len, txt in zip(motions, lengths, prompts)]
