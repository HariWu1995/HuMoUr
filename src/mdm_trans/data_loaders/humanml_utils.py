...
HML_LOWER_BODY_JOINTS = [HML_JOINT_NAMES.index(name) \
                                           for name in ['pelvis','left_hip','right_hip','left_knee','right_knee', 
...
class HumanMlNormalizer(Normalizer):
    ...
    def __init__(self, datapath=None):
        if datapath is not None:
            mean = torch.from_numpy(np.load(osp.join(datapath, 'Mean.npy'))).to(dist_util.dev())
            std  = torch.from_numpy(np.load(osp.join(datapath,  'Std.npy'))).to(dist_util.dev())
        else:
            mean = torch.from_numpy(np.load('dataset/humanml_mean.npy')).to(dist_util.dev())
            std  = torch.from_numpy(np.load('dataset/humanml_std.npy')).to(dist_util.dev())
