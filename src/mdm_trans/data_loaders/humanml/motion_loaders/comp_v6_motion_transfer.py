...
from src.mdm_trans.data_loaders.humanml.networks.modules import *
from src.mdm_trans.data_loaders.humanml.networks.trainers import CompTrainerV6
    ...
    def __init__(self, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, args):
        ...
        with torch.no_grad():
            for i, (_, model_kwargs) in tqdm(enumerate(dataloader)):
                ...
                if self.scale != 1.:
                    model_kwargs['y']['scale'] = torch.ones(cur_bs, device=dist_util.dev()) * self.scale
...
class CompTransferExternalDataset(CompTransferGeneratedDataset):
    ...
    def __init__(self, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, args):
        ...
        for sample_i in range(min(n_samples, real_num_samples)):
            sample_dict = {
                'motion': generated_motions[sample_i, -1].squeeze().transpose(1,0),
