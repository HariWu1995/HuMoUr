...
from src.mdm_prior.data_loaders.amass.file_io import read_json


# todo make it parametric I am too tired now

amass_path = os.environ.get('AMASS_DATASET_PATH', './data_loaders/babel/babel-smplh-30fps-male/amass.pth.tar')
amass_data = joblib.load(amass_path)
print(f'Loaded the dataset from {amass_path}')

babel_path = os.environ.get('AMASS_TO_BABEL_PATH', './data_loaders/babel-teach/id2fname/amass-path2babel.json')
amass2babel = read_json(babel_path)
print(f'Loaded the conversion from {babel_path}')
...
out_path = os.environ.get('OUTPUT_PATH', 'output-path/babel-smplh-30fps-male')
