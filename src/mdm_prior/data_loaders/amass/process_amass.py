...
BODY_MODEL_ROOT = os.environ.get('SMPL_DATA_ROOT', './body_models/smpl_models')
...
def get_body_model(model_type, gender, batch_size, device='cpu', ext='pkl'):
    ...
    body_model_path = f'{BODY_MODEL_ROOT}/{model_type}/{mtype}_{gender}.{ext}'
...
def process_sequence(filename, use_betas,
    ...
    while slice_ids[0] < new_num_frames:
        ...
        bodymodel_seq = get_body_model('smplh', 
...
def read_data(input_dir, model_type, output_dir, use_betas, gender):
    amass_subsets = [
                         f'{input_dir}/{x}' for x in AMASS_DIRS
