...
def get_in_between_mask(shape, lengths, prefix_end, suffix_end):
    ...
    for i, length in enumerate(lengths):
        start_idx = int(prefix_end * length)
        end_idx = int(suffix_end * length)
...
def get_prefix_mask(shape, prefix_length=20):
    ...
    prefix_mask = np.concatenate((np.ones((num_feat,           prefix_length)), 
                                 np.zeros((num_feat, seq_len - prefix_length))), axis=-1)
